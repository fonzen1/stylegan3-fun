...
def setup_snapshot_image_grid(training_set,  snap_res='8k', random_seed=0):
    size_dict = {'1080p': (1920, 1080, 3, 2), '4k': (
        3840, 2160, 7, 4), '8k': (7680, 4320, 7, 4)}
    rnd = np.random.RandomState(random_seed)
    gw = np.clip(size_dict[snap_res][0] //
                 training_set.image_shape[1], size_dict[snap_res][2], 32)
    gh = np.clip(size_dict[snap_res][1] //
                 training_set.image_shape[1], size_dict[snap_res][3], 32)
...
def training_loop(
    ...
    # Resolution size of the snapshot grid. Choose between [1080p | 4k | 8k]
    snap_res='8k',
    ...
    if rank == 0:
        ...
        grid_size, images, labels = setup_snapshot_image_grid(
            training_set=training_set, snap_res=snap_res)
        ...
        if G_ema.synthesis.img_channels == 4:
            rgb_images = images[:, :3, :, :]
            save_image_grid(rgb_images, os.path.join(
                run_dir, 'rgb_fakes_init.png'), drange=[-1, 1], grid_size=grid_size)
        elif G_ema.synthesis.img_channels == 2:
            la_images = images[:, :1, :, :]
            save_image_grid(la_images, os.path.join(
                run_dir, 'la_fakes_init.png'), drange=[-1, 1], grid_size=grid_size)
    ...
    while True:
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            ...
            if G_ema.synthesis.img_channels == 4:
                rgb_images = images[:, :3, :, :]
                save_image_grid(rgb_images, os.path.join(
                    run_dir, f'rgb_fakes{cur_nimg//1000:06d}.png'), drange=[-1, 1], grid_size=grid_size)
            elif G_ema.synthesis.img_channels == 2:
                la_images = images[:, :1, :, :]
                save_image_grid(rgb_images, os.path.join(
                    run_dir, f'la_fakes{cur_nimg//1000:06d}.png'), drange=[-1, 1], grid_size=grid_size)
